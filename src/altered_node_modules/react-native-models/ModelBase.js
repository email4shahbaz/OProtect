export default class ModelBase {
    ...
    constructor(properties) {
        for (const propertyName in properties) {
            //Shahbaz Alerted this to add each property from seperate function "addNewProperty"
            ...
            this.addNewProperty(propertyName, propertyType, propertyValue);
        }
    }


    //Shahbaz Created this function to add new property if id not exists
    //This is required when populatedFromState function is called
    //With a property that does not exist in this class yet.
    addNewProperty(propertyName, propertyType, val){
    ...
    populateFromState(state) {
        ...
        for (const propertyName in state) {
            ...
            const propertyNameCapitalized =
                ...
                //Shahbaz changed this code to add new property is it deos not exists
                //It also add getter and setter functions.
                if (!(privatePropertyName in this)) {
                  propertyVal = state[propertyName];
                  propertyType = ModelBase._getTypeName(propertyVal);
                  this.addNewProperty(propertyName, propertyType, propertyVal);

                  typeCheckObject[privatePropertyName] = null;
                  typeCheckObject["set" + propertyNameCapitalized] = (value) => {
                      if (ModelBase._checkType(value, propertyType) === false) {
                          throw new TypeError(`"${this.constructor.className}.${propertyName}" is of type "${ModelBase._getTypeName(value)}". Expected "${propertyType}".`);
                      }

                      typeCheckObject[privatePropertyName] = value;
                  };

                  typeCheckObject["get" + propertyNameCapitalized] = () => {
                      return typeCheckObject[privatePropertyName];
                  };
     ...
     */
     //Shahbaz changed this function if type==true then it returns
     //object without _ prefix.
    serialize(type) {
        return ModelBase._serialize(this, type);

        //return JSON.stringify(ModelBase._serialize(this, type));
    }
     ...
     * @returns {object}
     */
    static _serialize(object, removeDash) {
      console.log(removeDash);
        ...
        for (const key in object) {
            ...
            if (ModelBase._isObjectOrArray(value, "serialization")) {
                data[removeDash ? key.slice(1) : key] = ModelBase._processObjectOrArray(value, "serialization");
            } else {
                data[removeDash ? key.slice(1) : key] = ModelBase._processScalar(value, "serialization");
            }
        }

        container["data"] = data;
        return removeDash ? data :container;
    }
